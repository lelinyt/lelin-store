...
app.post('/login', async (req, res) => {
  ...
  if (!user || !pass) return res.status(400).json({ success: false, erro: 'Dados invalidos.' });
  ...
  registrarFalha(ip);
  return res.status(401).json({ success: false, erro: 'Usuario ou senha incorretos.' });
});
...
app.post('/produtos', async (req, res) => {
  ...
  if (!auth || !auth.startsWith('Bearer ')) return res.status(401).json({ erro: 'Nao autorizado.' });
  try {
